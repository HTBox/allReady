    ...
    public class SendRequestStatusToGetASmokeAlarmEndpointHandler : AsyncRequestHandler<SendRequestStatusToGetASmokeAlarmEndpoint>
    {
        ...
        protected override async Task HandleCore(SendRequestStatusToGetASmokeAlarmEndpoint message)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var response = await httpClient.PostAsJsonAsync($"admin/requests/status/{message.Serial}", updateRequestMessage);
            ...
            //            //return await HttpClient.PostAsJsonAsync($"admin/requests/status/{message.Serial}", json);
            //            //https://www.asp.net/web-api/overview/advanced/calling-a-web-api-from-a-net-client
            //            //"the PostAsJsonAsync method serializes an object to JSON and then sends the JSON payload in a POST request."
            //            //return await HttpClient.PostAsJsonAsync($"admin/requests/status/CHNI-16-00030", json);
            //            return await HttpClient.PostAsJsonAsync($"admin/requests/status/CHNI-16-00030", updateRequestMessage);
            //        }
            //    });
