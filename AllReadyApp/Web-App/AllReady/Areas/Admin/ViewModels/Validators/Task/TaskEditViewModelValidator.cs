...
namespace AllReady.Areas.Admin.ViewModels.Validators.Task
{
    public class TaskEditViewModelValidator : ITaskEditViewModelValidator
    {
        ...
        public async Task<List<KeyValuePair<string, string>>> Validate(EditViewModel viewModel)
        {
            ...
            {
                result.Add(new KeyValuePair<string, string>(nameof(viewModel.StartDateTime), "Start date cannot be earlier than the event start date " + parentEvent.StartDateTime.ToString("f")));
            }

            // Rule - End date cannot be out of range of parent event
            if (viewModel.EndDateTime > parentEvent.EndDateTime)
            {
                result.Add(new KeyValuePair<string, string>(nameof(viewModel.EndDateTime), "The end date of this task cannot be after the end date of the event " + parentEvent.EndDateTime.ToString("f")));
            }
